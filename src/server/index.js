...
${financialContext ? financialContext.slice(0, 12000) : 'No financial data available yet.'}`;
  ...
  try {
    ...
    const requestParams = { model: 'claude-sonnet-4-6', max_tokens: isPro ? 2048 : 1024, system: systemPrompt, messages };
...
app.get('/delete-account', (_req, res) => res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Delete Account — Walify AI</title><style>body{font-family:sans-serif;max-width:560px;margin:60px auto;padding:0 20px;color:#222}h1{font-size:24px}a{color:#238636}</style></head><body><h1>Delete Your Walify AI Account</h1><p>To permanently delete your account and all associated data, email us at:</p><p><a href="mailto:info@walify.ai">info@walify.ai</a></p><p>Please send from the email address associated with your account. We will process your request within 30 days and confirm deletion by email.</p><p>Deleting your account will permanently remove your financial data, transaction history, and all settings.</p></body></html>`));

// Digital Asset Links — required for TWA (Android app store)
// SHA-256 fingerprint comes from the keystore generated by PWA Builder
